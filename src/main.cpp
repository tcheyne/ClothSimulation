...
namespace
{
  ...
  void initSystem(int argc, char * argv[])
  {
    ...
    srand( time( NULL ) );
    if (!strncmp(argv[3],"p",1)) {
      cout << "Using PendulumSystem" << endl;
      system = new PendulumSystem(4);
    }
    else if (!strncmp(argv[1],"c",1)){
      cout << "Using ClothSystem" << endl;
      system = new ClothSystem();
    }
    else {
      cout << "Using SimpleSystem" << endl;
      system = new SimpleSystem();
    }
    if (!strncmp(argv[1],"e",1)) {
      cout << "Using Euler Integrator" << endl;
      timeStepper = new ForwardEuler();
    }
    else if (!strncmp(argv[1],"t",1)){
      cout << "Using Trapzoidal Integrator" << endl;
    ...
    string fs(argv[2]);
    stepSize = stof(fs);
    cout << "With a step size of " << stepSize << endl;
  }
